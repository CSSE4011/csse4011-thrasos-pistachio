...
K_MSGQ_DEFINE(ibeacon_msgq, sizeof(ibeacon_data_t), 20, 4);
...
void bt_advertiser_thread(void *p1, void *p2, void *p3) {
    ...
    ibeacon_data_t received_ibeacon_data;
    int err;
    ...
    while (1) {
        ...
        if (k_msgq_get(&ibeacon_msgq, &received_ibeacon_data, K_FOREVER) == 0) {
            ...
            sys_put_be16(received_ibeacon_data.major, &current_ibeacon_adv_data[20]);
            sys_put_be16(received_ibeacon_data.minor, &current_ibeacon_adv_data[22]);
            ...
            if (err) {
                ...
                printk("Advertising updated iBeacon (Major: %u, Minor: %u).\n",
                       received_ibeacon_data.major, received_ibeacon_data.minor);
